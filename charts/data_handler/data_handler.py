...
# top level function to get chart charts from local storage or external api
...
def generate_samples(asset_lists=ASSET_LISTS, samples_per_chart=20, normalize=False, prediction_interval=365):
    # initialize an empty array where all samples will be stored
    samples = pd.DataFrame()
    # go through all asset lists
    for asset_list in asset_lists:
        # get all names and symbols from the list
        asset_symbols = list(pd.read_csv(ASSET_LIST_PATH.format(asset_list), usecols=["Symbol"])["Symbol"])
